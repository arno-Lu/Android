package com.example.mechrevo.datebaseprovider;
...
public class DatabaseProvider extends ContentProvider {
    ...
    @Override
    public String getType(Uri uri) {                       //供系统调用，返回MIME类型
       switch (uriMatcher.match(uri)){
           case BOOK_DIR:
    ...
    public boolean onCreate() {
        dbHelp = new MyDatabaseHelper(getContext(),"BookStore.db",null,2);
        return true;
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)){
            case BOOK_DIR:
                ...
                break;
            case BOOK_ITEM:
                String bookId = uri.getPathSegments().get(1);  //得到uri的path部分，并拆分，去掉"/"，取到第一个元素(从第0个开始)。
